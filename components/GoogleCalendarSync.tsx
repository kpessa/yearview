...
export default function GoogleCalendarSync({
  ...
  useEffect(() => {
    ...
    if (googleCalendarService.handleRedirect()) {
      setIsSignedIn(true);
  ...
  const handleSignIn = () => {
    // Redirects to Google OAuth - page will reload when Google redirects back
    ...
    <div className={`flex ${isSidebar ? 'flex-col items-stretch' : 'items-center'} gap-2`}>
      {!isSignedIn ? (
        <button
          ...
          className="px-4 py-2 bg-white border border-neutral-300 rounded-xl text-sm font-medium text-neutral-700 hover:bg-neutral-50 transition-all flex items-center gap-2"
        >
